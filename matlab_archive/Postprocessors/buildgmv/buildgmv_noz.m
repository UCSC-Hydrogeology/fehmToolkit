function buildgmv_noz(rootname)
%
%USE: buildgmv ('rootname')
%
...
%   type. 
%   Revision 4/15 - this version zeros out z-vector prior to normalization
%   so that we can view just the horizontal dipolar nature of circulation.
%
...
% For this version, demarkate that z vectors are set to zero
%
      dotpos=findstr(scafin,'.');
      chars=scafin(dotpos+1:dotpos+5);
%      outfehm=strcat(rootname,'_',chars,'_bg.gmv');
      outfehm=strcat(rootname,'_',chars,'_bg-noz.gmv');
...
%
         ratio=4.54e-5;
         small=0.01;
% AF 4/14/15 Zero out vertical vector components prior to normaization.
%         vecavs(:,3)=1e-18;
         zabs=abs(vecavs(:,3));
         minz=min(zabs(zabs~=0));
         vecavs(:,3)=minz;
%         c=min(A(A~=0))
%
