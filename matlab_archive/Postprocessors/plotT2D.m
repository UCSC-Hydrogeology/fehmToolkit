function plotT2D(sy,xlimit,ylimit,tlim,interpmethod,sourcedir,lastflag)
...
%   Modified from plotT by A Fisher on 180221 for 2D simulations. **AF**
%
...
%**AF** xlims=[min(coor(:,2)),max(coor(:,2))];
xlims=[min(coor(:,1)),max(coor(:,1))];
%
ylims=[min(coor(:,3)),max(coor(:,3))];

%**AF** slice=node(coor(:,1)==sy);
slice=node(coor(:,2)==sy);
%
...
%**AF** g=TriScatteredInterp(coor(:,2),coor(:,3),T,interpmethod);
g=TriScatteredInterp(coor(:,1),coor(:,3),T,interpmethod);
%
% y=min(coor(:,2)):(max(coor(:,2))-min(coor(:,2)))/yres:max(coor(:,2));
y=min(coor(:,1)):(max(coor(:,1))-min(coor(:,1)))/yres:max(coor(:,1));
