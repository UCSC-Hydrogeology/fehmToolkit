function buildgmv2(rootname)
%
%USE: buildgmv2 ('rootname')
%
...
%
%   Revised by Tess Weathers, 2017/06/28
%       Updated for functionality without .iap files
%       Commented out lines 50-53, 100-110, 260-280
%       Updated all getprops to getprop2 (2017/08/03)
%
...
cond=getprop2(condfile);
fprintf('%s\t%s\n','Reading file: ',permfile)
perm=getprop2(permfile);
fprintf('%s\t%s\n','Reading file: ',rockfile)
rock=getprop2(rockfile);
fprintf('%s\t%s\n','Reading file: ',pporfile)
ppor=getprop2(pporfile);
%
...
%
         vnormx=vecnorm.*vecavs(:,1)./vectot;
         vnormy=vecnorm.*vecavs(:,2)./vectot;
