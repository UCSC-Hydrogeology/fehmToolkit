function mkfehmdir2 (root,outdir,sourcedir,useiap,uset0)
...
%   Revised 2017/07/26 by Tess Weathers
%       Removed dependencies on .ipi and .iap files
%       Updated to call fin2ini using only the root and source directories
%   Revised 2018/06 by Tess Weathers    
%       Updated to call fin2ini2 which does not require Temperature,
%       Pressure, or Saturation headers in .fin file
%       Updated to also copy .boun and .trac files if in sourcedir
%       Updated to call mkdotfiles2 which adds .trc to .files
...
if isempty(dir([sourcedir,'*.trac']))
    disp('No .trac file found, skipping...')
else
    disp('Locating TRACER INPUT (.trac) file...')
    tracfile=getfile([sourcedir,'*.trac']);
    sourcefiles=[sourcefiles;{tracfile}];
    outext=[outext;'.trac'];
end

if isempty(dir([sourcedir,'*.boun']))
    disp('No .boun file found, skipping...')
else
    disp('Locating BOUNDARY INPUT (.boun) file...')
    bounfile=getfile([sourcedir,'*.boun']);
    sourcefiles=[sourcefiles;{bounfile}];
    outext=[outext;'.boun'];
end
...
mkdotfiles2(root,[outdir,'fehmn.files'],restartflag);
mkdotfiles2(root,[outdir,root,'.files'],restartflag);%for use with old version of gofehml3, remove when all users are up to date
...
%            fin2ini([outdir,root,'.ini'],sourcedir,'top');
            fin2ini2([outdir,root,'.ini'],sourcedir);
%        end
    else
