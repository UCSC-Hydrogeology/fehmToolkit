...
%
%AF rockprop2 takes anisotropy from *.rpi file, see below
%
%FORMAT
...
%
%AF Define ANISOPERM as a vector for any material for which it is desired, using 
%AF this format: [I, J, K]. To get preferential horizontal flow by 10x, could use
%AF [1,1,0.1]. Note, in this case, use "baseline" permeability for X,Y, then Z is 1/10
%AF that value. Have not updated conductivity for this, but could be added later. 
%
...
for j=1:length(breaks)-1%loop over each equation definition
    clearvars FUN ZONES
    ANISOTROPY=[1,1,1];%default value for anisotropy if not specified
    for k=breaks(j)+1:breaks(j+1)-1, eval(rpi{k});end%define function from .rpi
...
for j=1:length(breaks)-1%loop over each equation definition
%AF   clearvars FUN ZONES
   clearvars FUN ZONES ANISOPERM
%AF    ANISOTROPY=[1,1,1];%default value for anisotropy
   if ~exist('ANISOPERM','var')
      ANISOPERM=[1,1,1];%default value for anisotropy if not specified
   end
%AF 
...
%AF        for k=1:3,permeability(zone_nodes,k)=ANISOTROPY(k).*perm;end
        for k=1:3,permeability(zone_nodes,k)=ANISOPERM(k).*perm;end
%AF           
    else
        warning('RockProp:MissingFunction',['No function defined in lines (',num2str(breaks(j)+1),'-',num2str(breaks(j+1)-1),') of ',root,'.rpi - skipping section...'])
    end
