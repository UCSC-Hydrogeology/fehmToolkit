...
%   Revision: 1.1, 2018/07/09 A. Fisher for 2d grids - needs some cleaning...or
%      perhaps could be combined with heatin so that it can check for 2d vs. 3d :)
%   Revision: 1.2 2018/08/20 A. Price added support for sorting values to
%       plot in 2d at the end of this script.
%
%INPUT
%----------------------
...
%Create line plot for visual check
fid=fopen([root,'.hflx']);
...
% put plot here...
[numhf,z]=size(hflx_plot);
y=hflx_plot./area(:,3);
x=min(coor(:,1)):(max(coor(:,1))-min(coor(:,1)))/(numhf-1):max(coor(:,1));
% Need to sort y values based on coordinates%
x=transpose(x);
y=y*10.^6;%convert to Watts
...
xlabel('x (m)');ylabel('HF');title('Heatflow (W/m^2)');
%
end